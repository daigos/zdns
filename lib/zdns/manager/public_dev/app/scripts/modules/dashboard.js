define([
    ...
    var Dashboard = zdns.module({ Models: {}, Collections: {} });
    ...
    Dashboard.Views.Layout = Backbone.View.extend({
        ...
        initialize: function() {
            ...
            this.listenTo( zdns.zone_list, 'remove', this.renderZoneList );
            this.listenTo( zdns.zone_list, 'change', this.renderZoneList );
        ...
        // Show/hide zone list
        toggleZoneList: function( ev ) {
            $(ev.target).closest('#toggle-zone-list').toggleClass('up');
            this.$zone_list.toggleClass('hide');
            return false;
        },
        ...
        renderZoneList: function() {
            ...
            tmpl = '<li class="zone" data-name="<%= name %>">' +
                    '<a href="#zone/<%= id %>/soa"><%= name %></a>' +
                    '</li>';
